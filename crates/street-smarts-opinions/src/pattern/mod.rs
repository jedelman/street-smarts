//! Pattern-presence opinions — one per Alexander pattern with a v0.1 operator.
//! Unlike the geometric chorus, these look directly for the thing the
//! pattern claims to produce, rather than proxying through general
//! wholeness properties.

pub mod p21_four_story_limit;
pub mod p22_nine_per_cent_parking;
pub mod p28_eccentric_nucleus;
pub mod p29_density_rings;
pub mod p32_shopping_street;
pub mod p36_degrees_of_publicness;
pub mod p38_row_houses;
pub mod p37_house_cluster;
pub mod p46_market_of_many_shops;
pub mod p48_housing_in_between;
pub mod p100_pedestrian_street;
pub mod p101_building_thoroughfare;
pub mod p112_entrance_transition;
pub mod p120_paths_and_goals;
pub mod p122_building_fronts;
pub mod p61_small_public_squares;
pub mod p89_corner_grocery;
pub mod p95_building_complex;
pub mod p106_positive_outdoor_space;
pub mod p108_connected_buildings;
pub mod p127_intimacy_gradient;
pub mod p128_indoor_sunlight;
pub mod p129_common_areas_at_the_heart;
pub mod p130_entrance_room;
pub mod p131_the_flow_through_rooms;
pub mod p133_staircase_as_a_stage;
pub mod p102_family_of_entrances;
pub mod p110_main_entrance;
pub mod p30_activity_nodes;
pub mod p49_looped_local_roads;
pub mod p50_t_junctions;
pub mod p60_accessible_green;
pub mod p105_south_facing_outdoors;
pub mod p121_path_shape;
pub mod p160_building_edge;
pub mod p99_main_building;
pub mod p114_hierarchy_of_open_space;
pub mod p115_courtyards_which_live;
pub mod p159_light_on_two_sides;
pub mod p163_outdoor_room;
pub mod p165_opening_to_the_street;
pub mod p191_shape_of_indoor_space;
pub mod p192_windows_overlooking_life;
pub mod p221_natural_doors_and_windows;
pub mod p67_common_land;
pub mod p52_network_of_paths_and_cars;
pub mod p107_wings_of_light;
pub mod p33_night_life;
pub mod p41_work_community;
pub mod p88_street_cafe;
pub mod p93_food_stands;
pub mod p123_pedestrian_density;
pub mod p139_farmhouse_kitchen;
pub mod p147_communal_eating;
pub mod p161_sunny_place;
pub mod p116_cascade_of_roofs;
pub mod p117_sheltering_roof;
pub mod p118_roof_garden;
pub mod p119_arcades;
pub mod p162_north_face;
pub mod p166_gallery_surround;
pub mod p51_green_streets;
pub mod p53_main_gateways;
pub mod p126_something_roughly_in_the_middle;
pub mod p124_activity_pockets;
pub mod p195_staircase_volume;
pub mod p164_street_windows;
pub mod p197_thick_walls;
pub mod p198_closets_between_rooms;
pub mod p31_promenade;
pub mod p59_quiet_backs;
pub mod p68_connected_play;
pub mod p98_circulation_realms;

pub use p21_four_story_limit::P21FourStoryLimit;
pub use p22_nine_per_cent_parking::P22NineParPercentParking;
pub use p28_eccentric_nucleus::P28EccentricNucleus;
pub use p29_density_rings::P29DensityRings;
pub use p32_shopping_street::P32ShoppingStreet;
pub use p36_degrees_of_publicness::P36DegreesOfPublicness;
pub use p38_row_houses::P38RowHouses;
pub use p37_house_cluster::P37HouseCluster;
pub use p46_market_of_many_shops::P46MarketOfManyShops;
pub use p48_housing_in_between::P48HousingInBetween;
pub use p100_pedestrian_street::P100PedestrianStreet;
pub use p101_building_thoroughfare::P101BuildingThoroughfare;
pub use p112_entrance_transition::P112EntranceTransition;
pub use p120_paths_and_goals::P120PathsAndGoals;
pub use p122_building_fronts::P122BuildingFronts;
pub use p61_small_public_squares::P61SmallPublicSquares;
pub use p89_corner_grocery::P89CornerGrocery;
pub use p95_building_complex::P95BuildingComplexOpinion;
pub use p106_positive_outdoor_space::P106PositiveOutdoorSpace;
pub use p108_connected_buildings::P108ConnectedBuildings;
pub use p127_intimacy_gradient::P127IntimacyGradient;
pub use p128_indoor_sunlight::P128IndoorSunlight;
pub use p129_common_areas_at_the_heart::P129CommonAreasAtTheHeart;
pub use p130_entrance_room::P130EntranceRoom;
pub use p131_the_flow_through_rooms::P131TheFlowThroughRooms;
pub use p133_staircase_as_a_stage::P133StaircaseAsAStage;
pub use p102_family_of_entrances::P102FamilyOfEntrances;
pub use p110_main_entrance::P110MainEntrance;
pub use p30_activity_nodes::P30ActivityNodes;
pub use p49_looped_local_roads::P49LoopedLocalRoads;
pub use p50_t_junctions::P50TJunctions;
pub use p60_accessible_green::P60AccessibleGreen;
pub use p105_south_facing_outdoors::P105SouthFacingOutdoors;
pub use p121_path_shape::P121PathShape;
pub use p160_building_edge::P160BuildingEdge;
pub use p99_main_building::P99MainBuilding;
pub use p114_hierarchy_of_open_space::P114HierarchyOfOpenSpace;
pub use p115_courtyards_which_live::P115CourtyardsWhichLive;
pub use p159_light_on_two_sides::P159LightOnTwoSides;
pub use p163_outdoor_room::P163OutdoorRoom;
pub use p165_opening_to_the_street::P165OpeningToTheStreet;
pub use p191_shape_of_indoor_space::P191ShapeOfIndoorSpace;
pub use p192_windows_overlooking_life::P192WindowsOverlookingLife;
pub use p221_natural_doors_and_windows::P221NaturalDoorsAndWindows;
pub use p67_common_land::P67CommonLand;
pub use p52_network_of_paths_and_cars::P52NetworkOfPathsAndCars;
pub use p107_wings_of_light::P107WingsOfLightOpinion;
pub use p33_night_life::P33NightLife;
pub use p41_work_community::P41WorkCommunity;
pub use p88_street_cafe::P88StreetCafe;
pub use p93_food_stands::P93FoodStands;
pub use p123_pedestrian_density::P123PedestrianDensity;
pub use p139_farmhouse_kitchen::P139FarmhouseKitchen;
pub use p147_communal_eating::P147CommunalEating;
pub use p161_sunny_place::P161SunnyPlace;
pub use p116_cascade_of_roofs::P116CascadeOfRoofs;
pub use p117_sheltering_roof::P117ShelteringRoof;
pub use p118_roof_garden::P118RoofGarden;
pub use p119_arcades::P119Arcades;
pub use p162_north_face::P162NorthFace;
pub use p166_gallery_surround::P166GallerySurround;
pub use p51_green_streets::P51GreenStreets;
pub use p53_main_gateways::P53MainGateways;
pub use p126_something_roughly_in_the_middle::P126SomethingRoughlyInTheMiddle;
pub use p124_activity_pockets::P124ActivityPockets;
pub use p195_staircase_volume::P195StaircaseVolume;
pub use p164_street_windows::P164StreetWindows;
pub use p197_thick_walls::P197ThickWalls;
pub use p198_closets_between_rooms::P198ClosetsBetweenRooms;
pub use p31_promenade::P31Promenade;
pub use p59_quiet_backs::P59QuietBacks;
pub use p68_connected_play::P68ConnectedPlay;
pub use p98_circulation_realms::P98CirculationRealms;
